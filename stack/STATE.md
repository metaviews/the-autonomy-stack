...
* **Stage 3 ecosystem governance has begun.** The Agent Ecosystem Audit now maps multi-agent relationships, providers, tools, memory stores, humans, affected parties, authority flows, data flows, and ecosystem-level contestability. The Stewardship Accountability Register names who can change prompts, protocols, access, memory, evaluation, provider choices, updates, and contestability paths. The Authority Migration Check identifies power that has accumulated through repeated use, dependency, defaults, or workflow centrality. The Authority Graph Schema gives agents a compact node-and-edge representation for these relationships.
* **Documentation validation is now part of agent-legibility.** `scripts/validate_docs.py` checks local Markdown links, toolkit metadata, tool index consistency, philosopher corpus paths, and source-corpus counts.
